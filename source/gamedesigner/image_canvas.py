...
def pixel_canvas_prompt_rules() -> str:
    return (
        ...
        "- 源图应按 ASE/Aseprite 式像素绘制思路组织：清晰轮廓、硬边色块、统一正方形像素格；如果用户指定 128x128 等目标尺寸，本地算法会等比完整放入目标画布并用透明边补齐，不会为了填满而裁掉主体。\n"
        "- 画面必须适合 1/4 或指定比例的严格网格采样：大形、线条、眼睛、手、道具、边缘都应落在稳定色块上，避免小于采样格的噪点细节；最终调色板会压缩到少量主色，避免过渡色污染。\n"
