...
def pixel_canvas_prompt_rules() -> str:
    return (
        "【像素作画硬约束】\n"
        "- 目标是专业像素游戏素材，不是普通插画加马赛克滤镜；先生成高品质、清晰、细节完整的源图，再由本地算法做轻量像素对齐清理。\n"
        "- 源图应按 ASE/Aseprite 式像素绘制思路组织：清晰轮廓、硬边色块、统一正方形像素格；如果用户指定 128x128 等目标尺寸，本地算法会等比完整放入目标画布并用透明边补齐，不会为了填满而裁掉主体；最终放大预览使用 nearest-neighbor。\n"
        "- 画面必须适合 1/4 或指定比例的严格网格采样：大形、线条、眼睛、手、道具、边缘都应落在稳定色块上，避免小于采样格的噪点细节；调色板应受控但不要过少，必须保留脸部、服装、道具和材质读性。\n"
