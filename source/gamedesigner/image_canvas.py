...
def pixel_canvas_prompt_rules() -> str:
    return (
        "【像素作画硬约束】\n"
        "- 目标是专业像素游戏素材，不是普通插画加马赛克滤镜；推荐流程是先生成 3x 或 4x 精确倍率清晰源图，再本地输出多种粗像素候选，最后让 AI 参考最佳候选重绘为原生像素稿。\n"
        "- 源图应按 ASE/Aseprite 式像素绘制思路组织：清晰轮廓、硬边色块、统一正方形像素格；如果用户指定 128x128、256x256、256x384 等最终尺寸，优先让 AI 源图成为该尺寸的 3x 或 4x。粗缩小候选只用于参考，不作为最终美术资产；最终放大预览使用 nearest-neighbor。\n"
        "- 画面必须即使不缩采样也像原生像素素材：大形、线条、眼睛、手、道具、边缘都应落在稳定色块上，避免噪点细节；调色板应受控但不要过少，必须保留脸部、服装、道具和材质读性。\n"
        "- 禁止抗锯齿、模糊采样、柔焦、高光晕、半透明羽化边、照片级渐变、过量细碎噪点和 AI 伪纹理。\n"
        "- 线条必须 pixel-perfect：硬边阶梯规律，曲线由干净像素簇构成，清除 L 形拐角夹心像素、双宽斜线、孤立脏点和只有 1-2 像素的边缘碎片。\n"
        "- 使用受控调色板：主色和色阶稳定，允许有控制的 hue shift；避免照片级过渡色污染，但不要把整张图压成脏灰、低色数或大块儿童拼图感。\n"
