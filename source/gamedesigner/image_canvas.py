...
def pixel_canvas_prompt_rules() -> str:
    return (
        "【像素作画硬约束】\n"
        "- 目标是专业像素游戏素材，不是普通插画加马赛克滤镜；AI 阶段就要生成高品质、清晰、细节完整、像素画思维明确的源图，本地默认只做轻量标记和透明整理。\n"
        "- 源图应按 ASE/Aseprite 式像素绘制思路组织：清晰轮廓、硬边色块、统一正方形像素格；只有用户明确指定 128x128 等像素输出尺寸时，本地算法才会等比完整放入目标画布并用透明边补齐，不会为了填满而裁掉主体；最终放大预览使用 nearest-neighbor。\n"
